...
class PulseGen(SoundGen):
    ...
    def generate_buffer(self, params: dict[str, float]) -> np.ndarray:
        ...
        if am != 0:
            ...
            am_envelope = 1. + 0.3*np.sin(am_phase)
...
class SineGen(SoundGen):
    ...
    def generate_buffer(self, params: dict[str, float]) -> np.ndarray:
        ...
        # パラメータを計算
        target_freq = 220*(4**self.params['frequency'])  # frequencyが0のとき220Hz、1のとき880Hz
...
class ContinuousSoundPlayer:
    ...
    def start(self, initial_params: dict[str, float] | None = None) -> None:
        ...
        """
        self.running = True
        self.sound_gen.reset_phase(initial_params)  # 開始時に位相をリセット（初期パラメータを渡す）
        ...
        while self.running:
            try:
                ...
                for key in self.current_params:
                    self.current_params[key] += (self.target_params[key] - self.current_params[key]) * self.interpolation_speed
...
if __name__ == "__main__":
    ...
    try:
        # パラメータを徐々に変更
        for i in range(101):
            player.update_param(i/100.) # 0.0~1.0の範囲でパラメータを変更
